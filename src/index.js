...
let botInfo = { username: 'robinbubble' };
...
async function handleGroupMessage(bot, msg) {
  try {
    ...
    try {
      ...
      // Use Base chain for all EVM addresses
      let chainToUse;
      
      if (contractAddress.startsWith('0x')) {
        // For EVM addresses, always use Base chain
        logger.debug(`Using Base chain for EVM address: ${contractAddress}`);
        chainToUse = await validation.detectEVMChain(contractAddress);
        
        // Fallback to default (Base) if needed
        if (!chainToUse) {
          logger.debug(`Using default Base chain`);
          chainToUse = constants.defaultChain;
        } else {
          logger.debug(`Using chain: ${chainToUse}`);
        }
      } else {
        // For non-EVM addresses (e.g., Solana), show error as we only support Base
        logger.debug(`Non-EVM address detected, Base chain only`);
        chainToUse = 'base'; // Still try Base, but it will likely fail validation
      }
      ...
      try {
        ...
        const botUsername = botInfo?.username || 'robinbubble';
